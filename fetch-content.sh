...
curl -fsSL \
	-H "Authorization: Bearer ${KURA_TOKEN}" \
	"${KURA_BASE}/api/v1/${KURA_PROJECT}/global?limit=10" \
	>data/globals-raw.json

python3 -c "
import json
with open('data/globals-raw.json') as f:
    body = json.load(f)
pages = [p for p in body['data'] if p.get('published')]
out = {p['slug']: p for p in pages}
with open('data/globals.json', 'w') as f:
    json.dump(out, f, indent=2)
print(f'Wrote {len(out)} pages')
"

rm -f data/globals-raw.json
